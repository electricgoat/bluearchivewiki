...
def get_clue_display_data(ctx: EventContext, clue):
    ...
    if clue.get('LocalizeEtcId') in data.etc_localization:
        if 'NameEn' not in localization or 'DescriptionEn' not in localization:
    ...
    if not description:
        ...
        if hint_localize_id in data.localization:
            ...
            if 'En' not in hint_localization:
                ctx.missing_localization.add_entry(hint_localization)
            ...
            if 'NameEn' not in hint_localization and 'DescriptionEn' not in hint_localization:
                ctx.missing_etc_localization.add_entry(hint_localization)
...
def generate_clue_info_table(ctx: EventContext, clues):
    ...
    for clue in clues:
        clue_data = get_clue_display_data(ctx, clue)
        wikitext += '|-\n'
...
def get_mode_cluesearch(ctx: EventContext, season_id: int) -> str:
