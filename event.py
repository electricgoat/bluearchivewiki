...
def parse_missions(season_id):
    ...
    for mission in data.event_content_mission.values():
        ...
        for index, _ in enumerate(mission['MissionRewardParcelType']):
            ...
            if mission['Category'] in ["Achievement", "EventAchievement"]:
...
def generate():
    ...
    #CCG
    wikitext_ccg = ''
    if (args['event_season'], "MiniGameCCG") in data.event_content_seasons:
        wikitext_ccg = get_mode_ccg(args['event_season'], data, characters, items, furniture, emblems, missing_localization, missing_code_localization, missing_etc_localization)
    ...
    wikitext_janken = ''
    if (args['event_season'], "MinigameJanken") in data.event_content_seasons:
        wikitext_janken = get_mode_janken(args['event_season'], data, characters, items, furniture, emblems, missing_localization, missing_code_localization, missing_etc_localization)
    ...
    if (args['event_season'], "MiniShop") in data.event_content_seasons:
        wikitext_shops = '==Mini Shop==\n<div style="display: flex; flex-flow: row wrap; align-items: flex-start; gap: 4px;">\n'
        template = env.get_template('events/template_shop.txt')
    ...
    wikitext += '\n=Mission Details & Rewards=\n' + wikitext_missions + wikitext_shops + wikitext_boxgacha + wikitext_milestones + wikitext_cardshop+ wikitext_fortunegacha + wikitext_field + wikitext_treasure + wikitext_dreammaker + wikitext_defense + wikitext_road + wikitext_ccg + wikitext_janken + wikitext_footer
