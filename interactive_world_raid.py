...
def get_raid_boss_data(group):
    ...
    boss_data['stage'] = data.interactive_world_raid_stage[group]
    ...
    boss_data['name'] = data.etc_localization[data.characters[boss_data['stage'][0]['RaidCharacterId']]['LocalizeEtcId']].get('NameEn', data.characters[boss_data['stage'][0]['RaidCharacterId']]['DevName'])
...
def generate():
    ...
    region = 'jp'
    #season = season_data[region].interactive_world_raid_season[args['season_id']]
    for season in season_data[region].interactive_world_raid_season.values():
        ...
        for group in season['OpenRaidBossGroupId']:          
            ...
            clear_rewards = get_world_raid_reward(data.interactive_world_raid_boss_group[group]['WorldBossClearRewardGroupId'])
            ...
            wikitext += template.render(boss_data=boss_data[group], raid_group_data=data.interactive_world_raid_boss_group[group], clear_rewards = clear_rewards, Card=Card)
